	...
	for {
		...
		case "REGISTER":
			if m["Authorization"] == "" {
				...
				log.Printf("secret %s name %s acl %s %t gigest:%s response:%s", secret, name, acl, m.CheckIP(saddr.IP, acl), m.Digest(secret), m.ValueFrom("Authorization", "response"))
		...
		case "INVITE":
			...
			// pickup
			log.Printf("tqcenglish INVITE pickup")
